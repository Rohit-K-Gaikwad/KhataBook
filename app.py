...
def add_user(database, table):
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            new_filename = secure_filename(
                filename + str(random.randint(10000, 99999)) + "." + file_extension
            )
    ...
    col_name = [description[0] for description in cur.description[1:-1]]
    return render_template(
        "add_user.html", table=table, database=database, col_name=enumerate(col_name)
    )
...
def edit_user(database, table, uid):
    ...
    if request.method == "POST":
        ...
        if file.filename == "":
            ...
            col_name = ",".join([f"{i}=?" for i in col_name])
            data = cur.execute(
                f"select FILE from {table} where UID=?", (uid,)
            ).fetchall()
